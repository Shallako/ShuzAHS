package com.shoulico.ahs.generator;

import com.shoulico.ahs.domain.model.GpsCoordinate;
import com.shoulico.ahs.domain.model.Location;
...
class TelemetryDataGeneratorTest {
    ...
    @Test
    @DisplayName("Should generate valid telemetry for Titan 300 truck")
    void testGenerateTitan300Telemetry() {
    ...
    @Test
    @DisplayName("Should generate valid telemetry for Titan 400 truck")
    void testGenerateTitan400Telemetry() {
        String vehicleId = "TITAN-400-001";
    ...
    @Test
    @DisplayName("Should generate different payloads for Titan 300 vs Titan 400")
    void testPayloadByModel() {
        VehicleTelemetry telemetryTitan300 = generator.generateTelemetry("TITAN-300-001", 
            VehicleStatus.HAULING);
        VehicleTelemetry telemetryTitan400 = generator.generateTelemetry("TITAN-400-001", 
            VehicleStatus.HAULING);
        
        // Titan 300 capacity: 300 tons (240-300 when loaded at 80-100%)
        assertTrue(telemetryTitan300.getPayloadTons() >= 240.0 
            && telemetryTitan300.getPayloadTons() <= 300.0);
        
        // Titan 400 capacity: 400 tons (320-400 when loaded at 80-100%)
        assertTrue(telemetryTitan400.getPayloadTons() >= 320.0 
            && telemetryTitan400.getPayloadTons() <= 400.0);
    }

    @Test
    @DisplayName("Should generate telemetry with location")
    void testTelemetryLocation() {
        VehicleTelemetry telemetry = generator.generateTelemetry("TITAN-300-001", 
            VehicleStatus.HAULING);
